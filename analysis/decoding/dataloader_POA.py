...
class TrialDataLoader:
    ...
    def __init__(self, key_path, batch_size, frc, crop=((9, 73), (0, 40), (13, 77)), content_root="./", device="cuda",
        ...
        self.uncolored_shape_subset = uncolored_shape_set.sample(frac=frc)
...
#         self.uncolored_shape_train = self.uncolored_subset[:int(3 * len(color_set) / 5)]
#         self.uncolored_shape_dev = self.uncolored_subset[int(3 * len(color_set) / 5):int(4 * len(color_set) / 5)]
#         self.uncolored_shape_test = self.uncolored_subset[int(4 * len(color_set) / 5):]
#         self.uncolored_shape_cross_dev = self.uncolored_subset[:int(len(color_set) / 2)]
#         self.uncolored_shape_cross_test = self.uncolored_subset[int(len(color_set) / 2):]
#
#         self.crop = crop
#         size = max([t[1] - t[0] for t in crop])
#         self.shape = (size, size, size)
