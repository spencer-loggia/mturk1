...
from neurotools.util import spearman_correlation
...
def get_subject_accs(acc_map, roi_dir, roi_keys, train_set, test_set,
                     thresh=0.22):
    ...
    for i, key in enumerate(roi_keys):
        ...
        train_stim_data = torch.from_numpy(train_stim_data)
        test_stim_data = torch.from_numpy(test_stim_data)
        dissim1 = dissimilarity_from_supervised(train_stim_data[None, :, :], torch.from_numpy(train_targets).int(), metric="cosine")
        dissim2 = dissimilarity_from_supervised(test_stim_data[None, :, :], torch.from_numpy(test_targets).int(), metric="cosine")

        corr = spearman_correlation(dissim1.squeeze(), dissim2.squeeze())
        cross_accs.append(corr)

    return np.abs(np.array(cross_accs))
...
if __name__ == '__main__':
    ...
    if SUBJECT == "wooster":
        ...
        ignore_sessions = []  # wooster NIF receive coil sessions
        crop = CROP_WOOSTER
    else:
    ...
    cross_corr = get_subject_accs(M1_S2C_ACC_MAP, M1_ROI_DIR, ROI_KEYS,
                                  train_set=shape_train_set,
                                  test_set=color_train_set)

    fig, ax = plt.subplots(1)
    fig.tight_layout()
    x_ax = np.arange(len(ROI_KEYS))
    ax.bar(x_ax, cross_corr)
    ...
    ax.set_xticks(np.arange(len(ROI_KEYS)))
    ax.set_xticklabels(ROI_KEYS, visible=True)
