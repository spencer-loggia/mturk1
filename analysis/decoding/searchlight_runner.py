...
if __name__ == '__main__':
    ...
    if STRATEGY == "binary":
        ...
        for i, c1 in enumerate(classes):
            for j, c2 in enumerate(classes[i + 1:]):
                ...
                if LOAD_MODEL is None:
                    ...
                    train_set = MTurk1.batch_iterator(abv_map[IN_SET], resample=False, num_train_batches=EPOCHS, n_workers=8,
                                                      standardize=True)
                    ...
                    # get in modality maps
                    train_set = MTurk1.batch_iterator(abv_map[IN_SET], resample=False, num_train_batches=100, n_workers=2,
                                                      standardize=True)
                    in_acc_map, in_ce_map = x_decoder.evaluate(train_set)
        ...
        # get in modality maps
        train_set = MTurk1.batch_iterator(abv_map[IN_SET], resample=False, num_train_batches=200, n_workers=2,
                                          standardize=True)
        in_acc_map, in_ce_map = x_decoder.evaluate(train_set)

        # get cross modality maps
        test_set = MTurk1.batch_iterator(abv_map[X_SET], resample=False, num_train_batches=200, n_workers=2,
                                         standardize=True)
        x_acc_map, x_ce_map = x_decoder.evaluate(test_set)
