...
class TrialDataLoader:
    ...
    one is served.
    """
    def __init__(self, key_path, batch_size, frc, crop=((9, 73), (0, 40), (13, 77)), content_root="./", device="cuda",
        ...
        color_set = self._get_set("colored_blobs", correct_only=use_behavior, balance=False) #.sample(frac=frc)
        print("Printing Color_Set in constructor for Trial DataLoader")
        print(color_set)
        self.color_all = color_set #self.color_all = color_set
        self.color_all
        #color_set.to_csv("/media/ssbeast/DATA/Users/KarthikKasi/Color_Full.csv")
        # Here we are trying to simulate giving the decoder access to only a small fraction of the overall dataset and using that to build the training and testing sets
        self.color_subset = color_set.sample(frac=frc)
        #self.color_subset.to_csv("/media/ssbeast/DATA/Users/KarthikKasi/Color_Sample.csv")
        self.color_train = self.color_subset[:int(4 * len(color_set) / 5)] #self.color_train = color_set[:int(4 * len(color_set) / 5)]
        self.color_test = self.color_subset[int(4 * len(color_set) / 5):] #self.color_test = color_set[int(4 * len(color_set) / 5):]
        self.color_cross_dev = self.color_subset[:int(len(color_set) / 2)]
        self.color_cross_test = self.color_subset[int(len(color_set) / 2):]

        uncolored_shape_set = self._get_set("uncolored_shapes", correct_only=use_behavior, balance=False) #.sample(frac=frc)
        print("Printing UnColor_Set in constructor for Trial DataLoader")
        print(uncolored_shape_set)
        self.uncolored_shape_all = uncolored_shape_set
        #uncolored_shape_set.to_csv("/media/ssbeast/DATA/Users/KarthikKasi/Shape_Full.csv")
        self.uncolored_subset = uncolored_shape_set.sample(frac=frc)
        #self.uncolored_subset.to_csv("/media/ssbeast/DATA/Users/KarthikKasi/Shape_Sample.csv")
        #self.uncolored_shape_train = self.uncolored_subset[:int(3 * len(self.uncolored_subset) / 5)]
        #self.uncolored_shape_dev = self.uncolored_subset[int(3 * len(self.uncolored_subset) / 5):int(4 * len(self.uncolored_subset) / 5)]
        self.uncolored_shape_train = self.uncolored_subset[:int(4 * len(self.uncolored_subset) / 5)]
        self.uncolored_shape_test = self.uncolored_subset[int(4 * len(self.uncolored_subset) / 5):]
        self.uncolored_shape_cross_dev = self.uncolored_subset[:int(len(self.uncolored_subset) / 2)]
        self.uncolored_shape_cross_test = self.uncolored_subset[int(len(self.uncolored_subset) / 2):]
    ...
    def _get_set(self, id, balance=False, correct_only=True):
        ...
        if correct_only:
            data = data[data["correct"] == 1].sample(frac=1).reset_index(drop=True)
            print("Printing Raw Data in Get_Set from data key")
            print(data)
        else:
    ...
    def data_generator(self, dset, noise_frac_var=.2, spatial_translation_var=.67, max_base_examples=4, cube=True):
        ...
        print("Options:")
        print(options)
        #options = sorted(list([2, 3, 6, 7, 10, 11]))
        ...
        print(class_dset)
        print("Printing len")
        print(len(class_dset))
        basis_idxs = np.random.randint(0, len(class_dset), size=(basis_dim,))
        print("LookHere")
        print(basis_idxs)
        #basis_idxs = np.random.randint([2, 3, 6, 7, 10, 11], size=(basis_dim,))
    ...
    def batch_iterator(self, data_type, num_train_batches=1000, return_all=False, standardize=False, resample=True, n_workers=16, cube=True,):
        ...
        print("Printing dset from batch_iterator")
        print(dset)
        if return_all:
            bs = len(dset)
        else:
