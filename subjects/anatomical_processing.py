...
#%% Imports
import sys
sys.path.append("/media/ssbeast/DATA/Users/sriramnr/monkey_fmri/bin")
from preprocess import antsCoreg
...
import time
monkeyDeepBrainF = '/media/ssbeast/DATA/Users/Stuart/PREEMACS/monkey_deepbrain'
...
def mri_nu_correct(p):
    ...
    cmd= 'mri_nu_correct.mni --ants-n4 --i %s --o %s --distance 24'%(p,outP)
    call(cmd,shell=True)
    return outP
...
def antsCoReg(fixedP,movingP,outP,initialTrsnfrmP,ltrns=['Affine','SyN'],n_jobs=2):
    ...
    antsCoreg(fixedP,
                 movingP,
                 outP,
                 outPref='antsReg', n_jobs=n_jobs)
    frwdTrnsP =  glob.glob(outF+'/Composite.h5')[0]
    return frwdTrnsP
...
def sphinx_fix(inP,toStd=False,outP=None,pos_string=None,visualize=False):
    ...
    if outP == None:
        ...
        outP = f+'/%s_%s'%('sphnx',n)
    if pos_string==None:
        cmd='mri_convert -i %s -o %s --sphinx'%(inP,outP)
    else:
        cmd='mri_convert -i %s -o %s --in_orientation %s'%(inP,outP,pos_string)
...
anatF = '/home/ssbeast/Projects/SS/monkey_fmri/MTurk1/subjects/' # Where are the T1s
dicomF = './'
...
#%% convert .mgz files to nifti files

mgzPs = glob.glob(anatF+'/*.mgz')

for i in range(0,len(mgzPs)):
    cmd = 'mri_convert %s %s/T1_%d.nii.gz'%(mgzPs[i],anatF,i)
    call(cmd,shell=True)
...
if sphinx_convert:
    ...
    for image in images:
        sphinx_fix(image,pos_string='RIA',visualize=False)
        
#If it does not work, the input string (pos_string) may be incorrect (is currently 'PIL'). 
#This depends on the scanner and the orientation in which the monkey is being scanned. 
#Generally, they are the same for the same set of scanned T1s. Check the orientation by 
#going to cmd and typing 'mri_info {filename} and look at the string in 'Orientation":'
#%% Find necessary starting files
t1Ps = list(np.sort(glob.glob(os.path.join(anatF, '*t1*.nii.gz')))) # Find T1s. You need T1s
...
#Using this, you can check the sizes of all the sphinx corrected files. They should be the same.
...
avgT2P = combine_wInModality(t2Ps,
                                nuCorrect=True,
                                outF=anatF,
                                imType='T2',
                                transforms='r')
...
dnnBrainP,dnnMaskP = dnn_skull_strip(avgT1_niiP, anatF, p=0.5)
...
# To do this we use nibabel. You can obtain the data matrix directly and set all
# the voxel values to 0.

cropped_nifti = nib.load('brain_mask_p0.500000.nii')
# affine is the spatial transform of the file. Can be used to resize for overlays as desired.
affine = cropped_nifti.affine 
header = cropped_nifti.header
data = cropped_nifti.get_fdata()
# Now you look at the data matrix, which is a 3D numpy array. It is usually in w x h x d format. 
# In order to view the data you can click on it in the variable explorer


#%% What did you save the edited mask as?
maskP = os.path.join(anatF, 'final_brainmask.nii.gz')

#%% Mask the T1
f,n = os.path.split(maskP)
maskedP = os.path.join(f,'%s.nii.gz'%subj)
...
templateP ='/media/ssbeast/DATA/Users/sriramnr/NMT_v2.0_asym/NMT_v2.0_asym_05mm/NMT_v2.0_asym_05mm_SS.nii.gz'
segP = '/media/ssbeast/DATA/Users/sriramnr/NMT_v2.0_asym/NMT_v2.0_asym_05mm/NMT_v2.0_asym_05mm_segmentation.nii.gz'
outF = '/media/ssbeast/DATA/Users/sriramnr/castor_raw_data_for_anatomical'
itkTxt, itkMan=itkSnapManual(maskedP,templateP,outF)


NMTinSubj = anatF+'/NMTinSubj.nii.gz'
fwtrnsP = antsCoReg(maskedP,itkMan,NMTinSubj,itkMan,n_jobs=10)
...
segInSubj = anatF + '/ants_transformed.nii'
antsApplyTransforms(WMseg_path,maskedP,segInSubj,[fwtrnsP,itkTxt],interp = 'NearestNeighbor') # interp = 'NearestNeighbor'

freeview([maskedP,segInSubj])

#%%

wmHandP = os.path.join(f,'wm_hand_edit.nii.gz')

#%%Trying to generate a smoother white matter hand edit

nii_file = nib.load(wmHandP)
orig_data = nii_file.get_fdata()
smooth_data = scipy.ndimage.gaussian_filter(orig_data, sigma = 1, mode = 'constant')

#%% testing

(a,b,c) = smooth_data.shape
for i in range(a):
    for j in range(b):
        for k in range(c): 
            if smooth_data[i,j,k] >= 0.5:
                smooth_data[i,j,k] = 1
            else:
                smooth_data[i,j,k] = 0
                
#%% testing pt 2; taking the file out

out = nib.Nifti1Image(smooth_data, affine=nii_file.affine, header=nii_file.header)
nib.save(out, "./wm_hand_edit_smooth.nii")

#%%Final step
surfing_safari(maskedP, steps='precon_3')
